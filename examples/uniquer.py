...
each bus name is unique and devoid of characters that openPDC cannot use.
File names currently hard coded
"""
import re
...
for line in f:
    if "$\\theta\\" in line:
        ...
        name = theta_name[6:] # remove "theta "
        if re.search(spec, name): # if name contains invalid characters
            ...
            line = re.sub(name, new_name, line) # correct the line
            name = new_name
